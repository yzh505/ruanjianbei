...
import styles from '../../../scss/agentManage/myResources/teacherPlanRes.module.scss';
import img from '../../../image/1.png'
import {getPrivateResource,getTextSum} from '../../../api/courseware'; 
...
const TeacherPlanRes = () => {
  ...
  const [total, setTotal] = useState(60); // 总数据量，实际应该从后端获取
  ...
  const fetchData=async (page)=>{
   ...
   const fetchSumData= async ()=>{
        try{
         const res =await getTextSum({userId:userInfo.uid,textType:'teachingplan'});
      ...
      <div className={styles.contentSection}>
          ...
          <>
            <div className={styles.videoGrid}>
              {pptResources.map((ppt) => (
                  <div className={styles.videoCard} onClick={()=>handleNavigation(ppt.textId)}>
                    <div className={styles.videoThumbnail}>
                       <div className={styles.videoPreview}>
                        <img src={ppt.textPicture} alt={ppt.textName} />
                        <div className={styles.duration}>
                          <FileOutlined /> {ppt.pages}页
                        </div>
                      ...
                      <h3>标题：{ppt.textName}</h3>
                      <p>介绍：{ppt.textIntroduction}</p>
                    </div>
                  </div>
              ))}
            </div>
        ...
        <div className={styles.paginationWrapper}>
          <Pagination
            ...
            showSizeChanger
            showQuickJumper
            showTotal={(total) => `共 ${total} 教案资源`}
            pageSizeOptions={['10', '12', '24', '36']}
          />
        </div>
      </div>
    </div>
